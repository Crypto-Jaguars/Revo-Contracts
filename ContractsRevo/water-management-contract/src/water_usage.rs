...
pub fn record_usage(
    ...
    if env.storage().persistent().has(&DataKey::Usage(usage_id.clone())) {
    ...
    env.storage().persistent().set(&DataKey::Usage(usage_id.clone()), &usage);
    ...
    env.storage().persistent().set(&farmer_usages_key, &farmer_usages);
    ...
    env.storage().persistent().set(&parcel_usages_key, &parcel_usages);
...
pub fn get_usage_report(
    ...
    for usage_id in usage_ids.iter() {
        if let Some(usage) = env.storage().persistent().get::<DataKey, WaterUsage>(&DataKey::Usage(usage_id.clone())) {
            ...
            if usage.timestamp >= period_start && usage.timestamp <= period_end {
                ...
                if parcel_id.is_none() || parcel_id.as_ref() == Some(&usage.parcel_id) {
                    ...
                    if let Some(threshold) = env.storage().persistent().get::<DataKey, WaterThreshold>(&DataKey::Threshold(usage.parcel_id.clone())) {
                        let daily_efficiency = utils::calculate_efficiency_score(usage.volume, threshold.daily_limit);
