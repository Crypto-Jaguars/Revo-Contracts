...
pub fn setup_test_environment() -> (Env, WaterManagementContractClient<'static>, Address, Address) {
    let env = Env::default();
...
pub fn simulate_sensor_data_hash(env: &Env, sensor_type: &str, reading: i128, timestamp: u64) -> BytesN<32> {
...
pub fn create_high_volume_test_data(env: &Env, count: usize) -> Vec<(BytesN<32>, BytesN<32>, BytesN<32>, i128)> {
    ...
    for i in 0..count {
        ...
        let data_hash = simulate_sensor_data_hash(env, "flow_sensor", 1000 + (i as i128 * 10), env.ledger().timestamp());
...
pub fn validate_usage_record(usage: &crate::datatypes::WaterUsage, expected_farmer: &Address, expected_parcel: &BytesN<32>) -> bool {
...
pub fn validate_incentive_record(incentive: &crate::datatypes::Incentive, expected_farmer: &Address) -> bool {
...
pub fn validate_alert_record(alert: &crate::datatypes::Alert, expected_farmer: &Address, expected_parcel: &BytesN<32>) -> bool {
...
pub fn create_scalability_test_data(env: &Env, farmer_count: usize, usage_per_farmer: usize) -> Vec<(Address, Vec<(BytesN<32>, BytesN<32>, BytesN<32>, i128)>)> {
    let mut test_data = Vec::new(env);
    ...
    for farmer_idx in 0..farmer_count {
        ...
        for usage_idx in 0..usage_per_farmer {
            ...
            let data_hash = simulate_sensor_data_hash(env, "sensor", 1000, env.ledger().timestamp());
...
pub fn create_edge_case_data(env: &Env) -> [(BytesN<32>, BytesN<32>, BytesN<32>, i128, &'static str); 5] {
    [
        (create_test_usage_id(env, 1), create_test_parcel_id(env, 1), create_test_data_hash(env, 1), 1, "Minimum volume"),
        (create_test_usage_id(env, 2), create_test_parcel_id(env, 2), create_test_data_hash(env, 2), 100000, "Maximum volume"),
        (create_test_usage_id(env, 3), create_test_parcel_id(env, 3), create_test_data_hash(env, 3), 0, "Zero volume"),
        (create_test_usage_id(env, 4), create_test_parcel_id(env, 4), create_test_data_hash(env, 4), -100, "Negative volume"),
        (create_test_usage_id(env, 5), create_test_parcel_id(env, 5), create_test_data_hash(env, 5), 150000, "Excessive volume"),
    ]
}
...
pub fn validate_contract_state_consistency(
    ...
    let parcel_count = parcel_usages.iter()
...
pub fn create_integration_test_data(env: &Env) -> IntegrationTestData {
    ...
    sensor_data.push_back(simulate_sensor_data_hash(env, "flow_sensor", 1000, env.ledger().timestamp()));
    sensor_data.push_back(simulate_sensor_data_hash(env, "pressure_sensor", 2000, env.ledger().timestamp()));
    sensor_data.push_back(simulate_sensor_data_hash(env, "temperature_sensor", 3000, env.ledger().timestamp()));
