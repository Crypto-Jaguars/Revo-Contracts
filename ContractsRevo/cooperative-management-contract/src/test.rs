...
fn test_submit_proposal() {
    ...
    let stored_proposal = env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .get::<DataKey, Proposal>(&proposal_key)
            .unwrap()
    });
...
fn test_trigger_emergency() {
    ...
    let emergency_reason =
        String::from_str(&env, "Natural disaster affecting cooperative resources");
    ...
    let stored_reason = env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .get::<DataKey, String>(&emergency_key)
            .unwrap()
    });
...
fn test_track_accountability() {
    ...
    env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .set(&reputation_key, &(reputation_points as i128));
    });
...
fn test_distribute_profits() {
    ...
    let distribution = env.as_contract(&contract_id, || {
        <CooperativeManagementContract as ProfitDistribution>::distribute_profits(
            env.clone(),
            total_profit,
            members.clone(),
        )
    ...
    let member1_balance = env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .get::<DataKey, i128>(&member1_balance_key)
            .unwrap_or(0)
    });
    ...
    let member2_balance = env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .get::<DataKey, i128>(&member2_balance_key)
            .unwrap_or(0)
    });
...
fn test_share_expenses() {
    ...
    env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .set(&member1_balance_key, &300i128);
        env.storage()
            .persistent()
            .set(&member2_balance_key, &300i128);
    });
    ...
    let expenses = env.as_contract(&contract_id, || {
        <CooperativeManagementContract as ProfitDistribution>::share_expenses(
            env.clone(),
            total_expense,
            members.clone(),
        )
...
fn test_pool_investment() {
    ...
    env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .set(&member_balance_key, &1000i128);
    });
    ...
    let initial_balance = env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .get::<DataKey, i128>(&member_balance_key)
            .unwrap_or(0)
    });

    // Pool investment
    env.as_contract(&contract_id, || {
        <CooperativeManagementContract as ProfitDistribution>::pool_investment(
            env.clone(),
            member1.clone(),
            investment_amount,
        )
    ...
    env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .set(&member_balance_key, &(initial_balance - investment_amount));
    });
    ...
    let member_investment = env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .get::<DataKey, i128>(&member_investment_key)
            .unwrap_or(0)
    });
...
fn test_process_automated_payments() {
    ...
    env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .set(&member_balance_key, &500i128);
    });
    ...
    env.as_contract(&contract_id, || {
        <CooperativeManagementContract as ProfitDistribution>::process_automated_payments(
            env.clone(),
            members.clone(),
            payment_amount,
        )
...
fn test_process_automated_payments_insufficient_funds() {
    ...
    env.as_contract(&contract_id, || {
        ...
        env.storage()
            .persistent()
            .set(&member_balance_key, &100i128); // Less than payment_amount
    });
