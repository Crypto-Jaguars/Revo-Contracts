...
fn test_purchase_event_handling() {
    ...
    assert!(events.iter().any(|e| e.key == (Symbol::new(&env, "nft_minted"), tx_id.clone())), "NFT minting event should be emitted");
}

#[test]
fn test_minting_conditions() {
    let (env, contract_id) = create_test_env();
    let client = TransactionNFTContractClient::new(&env, &contract_id);
    ...
    // Test minting with invalid conditions
    assert_panics!(client.mint_nft(&buyer, &buyer, &amount, &product), "Buyer and seller cannot be the same address");
    assert_panics!(client.mint_nft(&buyer, &seller, &0, &product), "Amount must be greater than zero");
}
