...
fn test_transaction_proof_generation_success() {
    ...
    let proof_exists = verify_transaction_proof_exists(
        &env, &contract_id, &buyer, &seller, amount, &product_bytes
    );
    assert!(proof_exists, "Transaction proof should exist after NFT creation");
...
fn test_transaction_proof_uniqueness() {
    ...
    let tx_id1 = env.as_contract(&contract_id, || {
        crate::proof::generate_transaction_proof(
            env.clone(), buyer.clone(), seller.clone(), amount, product_bytes.clone()
        )
    });
    ...
    assert_ne!(tx_id1, tx_id2, "Proof IDs should be unique even with same transaction data");
}
...
fn test_transaction_existence_verification() {
    ...
    assert!(!exists_before, "Transaction proof should not exist before NFT creation");
    ...
    assert!(exists_after, "Transaction proof should exist after NFT creation");
}
...
fn test_nft_transaction_linkage_validation() {
    ...
    assert_eq!(metadata.buyer, buyer, "NFT buyer should match transaction buyer");
    assert_eq!(metadata.seller, seller, "NFT seller should match transaction seller");
    assert_eq!(metadata.amount, amount, "NFT amount should match transaction amount");
    assert_eq!(metadata.product, product, "NFT product should match transaction product");
}
...
fn test_verification_of_nonexistent_transaction() {
    ...
    assert!(!exists, "Non-existent transaction should not be verified as existing");
}
...
fn test_verification_with_invalid_parameters() {
    ...
    let wrong_buyer_exists = verify_transaction_proof_exists(
        &env, &contract_id, &wrong_buyer, &seller, amount, &product_bytes
    );
    assert!(!wrong_buyer_exists, "Verification should fail with wrong buyer");
    ...
    let wrong_seller_exists = verify_transaction_proof_exists(
        &env, &contract_id, &buyer, &wrong_seller, amount, &product_bytes
    );
    assert!(!wrong_seller_exists, "Verification should fail with wrong seller");

    // Test verification with wrong amount
    let wrong_amount_exists = verify_transaction_proof_exists(
        &env, &contract_id, &buyer, &seller, amount + 1, &product_bytes
    );
    assert!(!wrong_amount_exists, "Verification should fail with wrong amount");
    ...
    let wrong_product_exists = verify_transaction_proof_exists(
        &env, &contract_id, &buyer, &seller, amount, &wrong_product_bytes
    );
    assert!(!wrong_product_exists, "Verification should fail with wrong product");
}
...
fn test_transaction_verification_across_multiple_nfts() {
    ...
    for ((buyer, seller, amount, product), tx_id) in transactions.iter().zip(tx_ids.iter()) {
        ...
        let metadata = client.get_nft_metadata(tx_id).unwrap();
        assert_eq!(&metadata.buyer, buyer, "NFT metadata should match transaction");
        assert_eq!(&metadata.seller, seller, "NFT metadata should match transaction");
        assert_eq!(metadata.amount, *amount, "NFT metadata should match transaction");
        assert_eq!(&metadata.product, product, "NFT metadata should match transaction");
    }
}
...
fn test_verification_consistency_over_time() {
    ...
    assert!(exists_immediately, "Transaction should exist immediately after creation");
    ...
    assert!(metadata_still_exists.is_some(), "NFT metadata should persist over time");

    // Verify NFT metadata is still consistent
    let metadata = client.get_nft_metadata(&tx_id).unwrap();
    assert_eq!(metadata.buyer, buyer, "NFT buyer should remain consistent over time");
    assert_eq!(metadata.seller, seller, "NFT seller should remain consistent over time");
}
...
fn test_transaction_proof_hash_integrity() {
    ...
    assert_eq!(tx_id1, tx_id2, "Proofs should be deterministic with same parameters and timestamp");
}
...
fn test_verification_with_edge_case_amounts() {
    ...
    for &amount in &edge_amounts {
        ...
        assert!(proof_exists, "Transaction proof should exist for amount: {}", amount);
        
        // Verify metadata consistency
        let metadata = client.get_nft_metadata(&tx_id).unwrap();
        assert_eq!(metadata.amount, amount, "Metadata amount should match for: {}", amount);
    }
}
...
fn test_verification_with_different_product_variations() {
    ...
    for (i, product) in products.iter().enumerate() {
        ...
        assert!(proof_exists, "Transaction proof should exist for product variation {}", i);
        
        // Verify metadata consistency
        let metadata = client.get_nft_metadata(&tx_id).unwrap();
        assert_eq!(&metadata.product, product, "Metadata product should match variation {}", i);
    }
}
...
fn test_bulk_verification_performance() {
    ...
    for (buyer, seller, amount, product, tx_id) in created_transactions {
        ...
        let metadata = client.get_nft_metadata(&tx_id).unwrap();
        assert_eq!(metadata.buyer, buyer, "Bulk NFT metadata should be consistent");
    }
}
...
fn test_verification_integration_with_purchase_flow() {
    ...
    assert!(proof_exists, "Transaction proof should exist for purchase review integration");
    
    // Step 3: Verify NFT contains all purchase details
    let metadata = client.get_nft_metadata(&tx_id).unwrap();
    assert_eq!(metadata.buyer, buyer, "Purchase buyer should match NFT owner");
    assert_eq!(metadata.seller, seller, "Purchase seller should match NFT seller");
    assert_eq!(metadata.amount, amount, "Purchase amount should match NFT amount");
    assert_eq!(metadata.product, product, "Purchase product should match NFT product");
...
fn test_timestamp_inclusion_in_verification() {
    ...
    let tx_id1 = env1.as_contract(&contract_id1, || {
        crate::proof::generate_transaction_proof(
            env1.clone(), buyer1.clone(), seller1.clone(), amount, product_bytes1.clone()
        )
    });

    let tx_id2 = env2.as_contract(&contract_id2, || {
        crate::proof::generate_transaction_proof(
            env2.clone(), buyer2.clone(), seller2.clone(), amount, product_bytes2.clone()
        )
    });

    // Since addresses and timestamps are different, proofs should be different
    assert_ne!(tx_id1, tx_id2, "Proofs should be different with different timestamps and addresses");
}
...
fn test_metadata_retrieval_for_nonexistent_nft() {
    ...
    assert!(metadata.is_none(), "Non-existent NFT should return None metadata");
}
...
fn test_high_volume_nft_creation_scalability() {
    ...
    assert_eq!(created_nfts.len(), volume, "All high-volume NFTs should be created");
    ...
    for &i in &check_indices {
        ...
        assert!(proof_exists, "High-volume transaction proof should exist at index {}", i);
    }
}
...
fn test_concurrent_verification_operations() {
    ...
    for (tx_id, verified) in verification_results {
        assert!(verified, "Concurrent verification should succeed for NFT {:?}", tx_id);
    }
}
...
fn test_verification_system_integrity() {
    ...
    verify_nft_metadata(&metadata_unwrapped, &buyer, &seller, amount, &product, 12345);
