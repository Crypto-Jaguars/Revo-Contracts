...
fn test_nft_creation_with_metadata_validation() {
    ...
    assert_eq!(metadata.timestamp, 12345, "Timestamp should match ledger time");
}
...
fn test_nft_creation_with_different_amounts() {
    ...
    for &amount in &test_amounts {
        ...
        let metadata = client.get_nft_metadata(&tx_id).unwrap();
        assert_eq!(metadata.amount, amount, "Amount should match for value: {}", amount);
    }
}
...
fn test_nft_creation_with_different_products() {
    ...
    for product in products {
        ...
        assert_eq!(metadata.product, product, "Product should match in metadata");
...
fn test_nft_creation_event_emission() {
    ...
    assert!(nft_minted_event_exists, "NFT minting event should be emitted");
}
...
fn test_nft_creation_timestamp_accuracy() {
    ...
    assert_eq!(metadata.timestamp, timestamp, "Timestamp should match ledger timestamp");
}
...
fn test_nft_creation_with_time_progression() {
    ...
    assert!(metadata2.timestamp > metadata1.timestamp, "Second NFT should have later timestamp");
    assert_ne!(tx_id1, tx_id2, "NFTs should have different IDs even with same parties");
}
...
fn test_nft_creation_different_buyer_seller_combinations() {
    ...
    assert_eq!(nft_ids.len(), 12, "Should have created 12 unique NFTs (4*3 combinations)");
}
...
fn test_nft_metadata_retrieval_nonexistent() {
    ...
    assert!(metadata.is_none(), "Should return None for non-existent NFT");
}
...
fn test_nft_creation_metadata_consistency() {
    ...
    for (buyer, seller, amount, product) in transactions {
        ...
        assert_eq!(metadata.buyer, metadata2.buyer, "Buyer should be consistent");
        assert_eq!(metadata.seller, metadata2.seller, "Seller should be consistent");
        assert_eq!(metadata.amount, metadata2.amount, "Amount should be consistent");
        assert_eq!(metadata.product, metadata2.product, "Product should be consistent");
        assert_eq!(metadata.timestamp, metadata2.timestamp, "Timestamp should be consistent");
    }
}
...
fn test_nft_creation_large_amounts() {
    ...
    for &amount in &large_amounts {
        ...
        assert_eq!(metadata.amount, amount, "Large amount should be stored correctly");
    }
}
...
fn test_nft_creation_edge_case_products() {
    ...
    for product in edge_products {
        ...
        assert_eq!(metadata.product, product, "Edge case product should be stored correctly");
    }
}