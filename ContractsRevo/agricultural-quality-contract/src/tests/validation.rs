...
//     };
//     use crate::{AgricQualityContract, AgricQualityContractClient, CertificationData, CertificationStatus, QualityStandard, InspectionReport, DataKey::Inspection, AgricQualityError};
...
//         let (env, _, agric_client, _, cert_client, _, farmer, inspector, authority) = setup_integration_test();
...
//         let _ = agric_client.record_inspection(&inspector, &cert_id, &metrics, &findings, &recommendations);

//         // Step 6: Check compliance with invalid score
//         let report_result = agric_client.check_compliance(&cert_id, &inspector);

//         let _ = agric_client.process_certification(&authority, &cert_id, &false, &(365 * 24 * 60 * 60));
...
//         let verification_hash = create_document_hash(&env, "Organic certification document");
//         cert_client.issue_certification(&authority, &farmer, &cert_type, &expiration_date, &verification_hash);
...
//         let (env, _, agric_client, _, cert_client, _, farmer, _, authority) = setup_integration_test();
...
//         let result = agric_client.process_certification(&authority, &cert_id, &true, &(365 * 24 * 60 * 60));
//     }
...
//         let inspection_result = agric_client.record_inspection(&inspector, &cert_id, &metrics, &findings, &recommendations);

//         // Step 4: Check compliance with invalid score
//         let report_result = agric_client.check_compliance(&cert_id, &inspector);
//         assert_eq!(report_result.inspector, inspector, "Inspector should match");
//         assert_eq!(report_result.metrics.len(), 1, "One metric should be recorded");
//         assert_eq!(report_result.metrics.get(0).unwrap().0, metric_name, "Metric name should match");
//         assert_eq!(report_result.metrics.get(0).unwrap().1, 0u32, "Metric score should be 0 due to Organic standard adjustment");
//         assert_eq!(report_result.overall_score, 0u32, "Overall score should be 0 due to failing score");
//         assert_eq!(
...
//         assert_eq!(agric_certs.len(), 1, "One certification should be recorded");
//         let agric_cert = agric_certs.get(0).unwrap();
//         assert_eq!(agric_cert.status, CertificationStatus::Pending, "Certification should remain pending");
//     }
...
//         cert_client.issue_certification(&authority, &other_farmer, &cert_type, &expiration_date, &verification_hash);
...
//         let (env, _, agric_client, _, _, _, farmer, inspector, authority) = setup_integration_test();
...
//         let findings = vec![&env, String::from_str(&env, "No GMO traces detected")];
//         let recommendations = vec![&env, String::from_str(&env, "Maintain current practices")];
//         let inspection_result = agric_client.record_inspection(&unauthorized_inspector, &cert_id, &metrics, &findings, &recommendations);
//     }
