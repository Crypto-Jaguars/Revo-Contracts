use crate::datatype::{
    ChainlinkPriceData, ChainlinkPriceFeed, DataKey, FarmerCrop, PriceData, StabilizationError,
    StabilizationFund,
};
...
impl PriceUtils {
        ...
        for farmer_address in farmers.iter() {
            ...
            if env.storage().persistent().has(&farmer_crop_key) {
                let farmer_crop = env
                    .storage()
                    .persistent()
                    .get::<DataKey, FarmerCrop>(&farmer_crop_key);
    ...
    pub fn get_chainlink_feed(
        env: &Env,
        crop_type: &String,
    ) -> Result<ChainlinkPriceFeed, StabilizationError> {
    ...
    pub fn get_chainlink_price_data(
        env: &Env,
        crop_type: &String,
    ) -> Result<ChainlinkPriceData, StabilizationError> {
    ...
    pub fn is_chainlink_feed_active(
        env: &Env,
        crop_type: &String,
    ) -> Result<bool, StabilizationError> {
