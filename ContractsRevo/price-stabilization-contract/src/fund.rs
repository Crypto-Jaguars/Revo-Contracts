...
use soroban_sdk::{contractimpl, Address, BytesN, Bytes, Env, Map, String};
...
impl FundManagement for PriceStabilizationContract {
    fn create_fund(
        ...
        if env.storage().persistent().has(&DataKey::Fund(fund_id.clone().into())) {
        ...
        // Store the fund
        env.storage().persistent().set(&DataKey::Fund(fund_id.clone().into()), &fund);
    ...
    fn contribute_fund(
        ...
        env.storage().persistent().set(&contributor_key, &contributor_record);
    ...
    fn get_fund_status(
        ...
        status.set(String::from_str(&env, "price_threshold"), fund.price_threshold);
        
        // Add timestamp information
        status.set(String::from_str(&env, "creation_time"), fund.creation_time as i128);
        if let Some(last_payout) = fund.last_payout_time {
            status.set(String::from_str(&env, "last_payout_time"), last_payout as i128);
        }
