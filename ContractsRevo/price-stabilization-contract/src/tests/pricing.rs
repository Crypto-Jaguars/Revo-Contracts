...
fn test_oracle_registration_unauthorized() {
    ...
    assert!(result.is_err(), "unauthorized oracle registration should fail");
}
...
fn test_market_price_update_invalid_oracle() {
    ...
    let result = client.try_update_market_price(&unregistered_oracle, &crop_type, &price, &timestamp);
    assert!(result.is_err(), "price update from unregistered oracle should fail");
}
...
fn test_price_retrieval() {
    ...
    let timestamp = env.ledger().timestamp();
    client.try_update_market_price(&oracle, &crop_type, &expected_price, &timestamp).unwrap();
    ...
    match price_result {
        Ok(inner_result) => match inner_result {
            Ok((actual_price, _timestamp)) => {
                assert_eq!(actual_price, expected_price, "retrieved price should match set price");
            },
...
fn test_price_threshold_check_above() {
    ...
    let timestamp = env.ledger().timestamp();
    client.try_update_market_price(&oracle, &crop_type, &high_price, &timestamp).unwrap();
...
fn test_price_threshold_check_below() {
    ...
    let timestamp = env.ledger().timestamp();
    client.try_update_market_price(&oracle, &crop_type, &low_price, &timestamp).unwrap();
...
fn test_chainlink_integration() {
    ...
    assert!(result.is_ok(), "Chainlink oracle registration should succeed");
    
    // Test price feed from Chainlink
    let price = 11500i128;
    let timestamp = env.ledger().timestamp();
    let feed_result = client.try_update_market_price(&chainlink_oracle, &crop_type, &price, &timestamp);
...
fn test_price_data_validation_edge_cases() {
    ...
    let result2 = client.try_update_market_price(&oracle, &crop_type, &normal_price, &future_timestamp);
