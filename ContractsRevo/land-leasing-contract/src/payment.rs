...
use crate::leasing::{get_lease_agreement, update_next_payment_due, increment_payments_made, update_lease_status};
...
pub fn process_lease_payment(
    ...
    assert!(amount >= lease.payment_amount, "Insufficient payment amount");
    
    // Determine payment type
    let current_time = env.ledger().timestamp();
    let payment_type = if current_time > lease.next_payment_due {
        String::from_str(env, "Late")
    } else if current_time < lease.next_payment_due - 86400 { // 1 day early
...
fn store_payment_record(env: &Env, lease_id: &BytesN<32>, payment_record: &PaymentRecord) {
    ...
    env.storage().persistent().set(&(PAYMENT_HISTORY, lease_id.clone()), &payment_history);
}
