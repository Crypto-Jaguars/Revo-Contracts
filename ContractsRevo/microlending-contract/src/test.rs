...
fn test_funding_with_insufficient_balance() {
    ...
    mint_tokens(&env, &env.register_stellar_asset_contract(Address::generate(&env)), &lender_with_insufficient_balance, 500); // Low balance
...
fn test_repayment_with_insufficient_balance() {
    ...
    let result = client.try_repay_loan(&borrower_with_insufficient_balance, &loan_id, &per_installment);
    match result {
        Err(Ok(e)) if e == MicrolendingError::Unauthorized.into() => (),
        _ => panic!("Expected Unauthorized error (borrower mismatch), got: {:?}", result),
    }
}
...
fn test_repayment_exceeds_due_amount() {
    ...
    match result {
        ...
        _ => panic!("Expected RepaymentScheduleViolation error, got: {:?}", result),
    }
}
...
fn test_early_full_repayment() {
    ...
    env.ledger().with_mut(|li| li.timestamp += 15 * 24 * 60 * 60); // 15 days
...
fn test_default_status_check_accuracy() {
    ...
    env.ledger().with_mut(|li| li.timestamp += due_time_from_funded);
