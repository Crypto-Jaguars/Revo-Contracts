...
use crate::datatype::{
    PurchaseReviewError, ReviewDetails, PurchaseVerificationData,
    DataKeys, ProductRatings
};
...
impl PurchaseReviewContract {
    ...
    pub fn get_admin(env: Env) -> Result<Address, PurchaseReviewError> {
        env.storage().instance().get(&DataKeys::Admin)
    ...
    pub fn is_purchase_verified(
        ...
        let verification_data = env.storage().persistent()
    ...
    pub fn get_product_rating(
        ...
        if let Some(ratings) = env.storage().persistent().get::<_, ProductRatings>(&reviews_key) {
