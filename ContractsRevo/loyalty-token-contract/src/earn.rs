...
pub fn award_points(env: &Env, program_id: BytesN<32>, user_address: Address, transaction_amount: u32) {

    let program_key = (Symbol::new(env, "program"), program_id.clone());

    let program: LoyaltyProgram = env.storage()
        .persistent()
    ...
    let current_points: u64 = env.storage()
        .persistent()
    ...
    env.events().publish(
        (Symbol::new(env, "points_awarded"), program_id.clone(), user_address),
        points_to_award
    );
