...
# Agricultural Quality Contract Deployment Script
# Usage: ./deploy_agricultural_quality.zsh [network] [identity]
# Example: ./deploy_agricultural_quality.zsh testnet default
...
# Configuration
CONTRACT_NAME="agricultural-quality-contract"
SCRIPT_DIR="${0:A:h}"
PROJECT_ROOT="${SCRIPT_DIR:h}"
CONTRACT_DIR="${PROJECT_ROOT}/ContractsRevo/agricultural-quality-contract"
# Modern Stellar CLI uses workspace root target directory with wasm32v1-none
WASM_PATH="${PROJECT_ROOT}/target/wasm32v1-none/release/agricultural_quality_contract.wasm"
...
setup_logging() {
    ...
    # Initialize log file
    echo "Agricultural Quality Contract Deployment Log" > "$DEPLOYMENT_LOG"
...
build_contract() {
    ...
    if [[ ! -f "$WASM_PATH" ]]; then
        ...
        ALT_WASM_PATH="${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/agricultural_quality_contract.wasm"
        if [[ -f "$ALT_WASM_PATH" ]]; then
            ...
            echo "  - ${PROJECT_ROOT}/target/wasm32v1-none/release/agricultural_quality_contract.wasm"
            echo "  - ${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/agricultural_quality_contract.wasm"
...
Agricultural Quality Contract Deployment Summary
================================================
...
print_usage() {
    echo "Usage: ./deploy_agricultural_quality.zsh [network] [identity]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  ./deploy_agricultural_quality.zsh testnet                  # Deploy to testnet with default identity"
    echo "  ./deploy_agricultural_quality.zsh testnet alice            # Deploy to testnet with 'alice' identity"
    echo "  ./deploy_agricultural_quality.zsh mainnet production       # Deploy to mainnet with production identity"
    echo ""
    echo "Prerequisites:"
    ...
    echo "  1. Build the agricultural quality contract"
