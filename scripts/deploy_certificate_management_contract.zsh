...
# Certificate Management Contract Deployment Script
# Usage: ./deploy_certificate_management_contract.zsh [network] [identity]
# Example: ./deploy_certificate_management_contract.zsh testnet default
...
CONTRACT_DIR="${PROJECT_ROOT}/ContractsRevo/certificate-management-contract"
# Modern Stellar CLI uses workspace root target directory with wasm32v1-none
WASM_PATH="${PROJECT_ROOT}/target/wasm32v1-none/release/certificate_management_contract.wasm"
...
check_prerequisites() {
    ...
    if ! command -v jq &> /dev/null; then
        print_warning "jq is not installed. JSON parsing may be limited."
...
setup_logging() {
    ...
    # Initialize log file
    echo "Certificate Management Contract Deployment Log" > "$DEPLOYMENT_LOG"
...
build_contract() {
    ...
    if [[ ! -f "$WASM_PATH" ]]; then
        ...
        ALT_WASM_PATH="${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/certificate_management_contract.wasm"
        if [[ -f "$ALT_WASM_PATH" ]]; then
            ...
            ALT_WASM_PATH2="${PROJECT_ROOT}/target/wasm32-unknown-unknown/release/certificate_management_contract.wasm"
            if [[ -f "$ALT_WASM_PATH2" ]]; then
                ...
                echo "  - ${PROJECT_ROOT}/target/wasm32v1-none/release/certificate_management_contract.wasm"
                echo "  - ${PROJECT_ROOT}/target/wasm32-unknown-unknown/release/certificate_management_contract.wasm"
                echo "  - ${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/certificate_management_contract.wasm"
...
Certificate Management Contract Deployment Summary
===================================================
...
display_results() {
    ...
    echo "  1. Initialize the contract with an admin address"
    echo "     stellar contract invoke \\"
    echo "       --id $CONTRACT_ID \\"
    echo "       --source-account $PROFILE \\"
    echo "       --network $NETWORK \\"
    echo "       -- initialize \\"
    echo "       --admin <ADMIN_ADDRESS>"
    echo ""
    echo "  2. Verify deployment on Stellar Explorer"
    echo "  3. Test contract functionality"
    echo "  4. Issue test certificates"
    echo ""
...
print_usage() {
    echo "Usage: ./deploy_certificate_management_contract.zsh [network] [identity]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  ./deploy_certificate_management_contract.zsh testnet                  # Deploy to testnet with default identity"
    echo "  ./deploy_certificate_management_contract.zsh testnet alice            # Deploy to testnet with 'alice' identity"
    echo "  ./deploy_certificate_management_contract.zsh mainnet production       # Deploy to mainnet with production identity"
    echo ""
    echo "Prerequisites:"
    ...
    echo "  1. Build the certificate management contract"
