...
# Crop Yield Prediction Contract Deployment Script
# Usage: ./deploy_crop_yield_prediction.zsh [network] [identity]
# Example: ./deploy_crop_yield_prediction.zsh testnet default
...
CONTRACT_DIR="${PROJECT_ROOT}/ContractsRevo/crop-yield-prediction"
# Modern Stellar CLI uses workspace root target directory with wasm32v1-none
WASM_PATH="${PROJECT_ROOT}/target/wasm32v1-none/release/crop_yield_prediction.wasm"
...
setup_logging() {
    ...
    # Initialize log file
    echo "Crop Yield Prediction Contract Deployment Log" > "$DEPLOYMENT_LOG"
...
build_contract() {
    ...
    if [[ ! -f "$WASM_PATH" ]]; then
        ...
        ALT_WASM_PATH="${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/crop_yield_prediction.wasm"
        if [[ -f "$ALT_WASM_PATH" ]]; then
            ...
            echo "  - ${PROJECT_ROOT}/target/wasm32v1-none/release/crop_yield_prediction.wasm"
            echo "  - ${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/crop_yield_prediction.wasm"
...
Crop Yield Prediction Contract Deployment Summary
=================================================
...
print_usage() {
    echo "Usage: ./deploy_crop_yield_prediction.zsh [network] [identity]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  ./deploy_crop_yield_prediction.zsh testnet                    # Deploy to testnet with default identity"
    echo "  ./deploy_crop_yield_prediction.zsh testnet alice              # Deploy to testnet with 'alice' identity"
    echo "  ./deploy_crop_yield_prediction.zsh mainnet production         # Deploy to mainnet with production identity"
    echo ""
    echo "Prerequisites:"
    ...
    echo "  1. Build the crop yield prediction contract"
