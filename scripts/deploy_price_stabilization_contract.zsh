...
show_usage() {
    ...
    echo -e "  ${CYAN}./scripts/deploy_price_stabilization_contract.zsh [network] [profile]${NC}"
    ...
    echo -e "  ${CYAN}./scripts/deploy_price_stabilization_contract.zsh testnet kennyv4${NC}"
    echo -e "  ${CYAN}./scripts/deploy_price_stabilization_contract.zsh mainnet production${NC}"
}

# Check if parameters are provided
if [ $# -ne 2 ]; then
    print_header "Price Stabilization Contract Deployment"
...
# Price Stabilization Contract deployment
print_header "Deploying Price Stabilization Contract"

log_step "Uploading price_stabilization_contract to $NETWORK..."
price_stabilization_contract_upload_output=$(stellar contract upload \
  --network $NETWORK \
  --source $PROFILE \
  --wasm target/wasm32v1-none/release/price_stabilization_contract.wasm)
...
log_info "WASM Hash: ${BOLD}$price_stabilization_contract_upload_output${NC}"

log_step "Deploying price_stabilization_contract..."
price_stabilization_contract_deploy_output=$(stellar contract deploy \
  --wasm-hash $price_stabilization_contract_upload_output \
  --source $PROFILE \
  --network $NETWORK \
  --alias PriceStabilizationContract)
...
log_success "Price Stabilization Contract deployed successfully!"
log_info "Contract ID: ${BOLD}$price_stabilization_contract_deploy_output${NC}"
log_info "Network: ${BOLD}$NETWORK${NC}"
log_info "Profile: ${BOLD}$PROFILE${NC}"
