...
# Environmental Impact Tracking Contract Deployment Script
# Usage: ./deploy_environmental_impact.zsh [network] [identity]
# Example: ./deploy_environmental_impact.zsh testnet default
...
CONTRACT_DIR="${PROJECT_ROOT}/ContractsRevo/environmental-impact-tracking"
# Stellar CLI uses workspace root target directory with wasm32v1-none
WASM_PATH="${PROJECT_ROOT}/target/wasm32v1-none/release/environmental_impact_tracking.wasm"
...
setup_logging() {
    ...
    # Initialize log file
    echo "Environmental Impact Tracking Contract Deployment Log" > "$DEPLOYMENT_LOG"
...
build_contract() {
    ...
    if [[ ! -f "$WASM_PATH" ]]; then
        ...
        ALT_WASM_PATH="${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/environmental_impact_tracking.wasm"
        if [[ -f "$ALT_WASM_PATH" ]]; then
            ...
            ALT_WASM_PATH2="${PROJECT_ROOT}/target/wasm32-unknown-unknown/release/environmental_impact_tracking.wasm"
            if [[ -f "$ALT_WASM_PATH2" ]]; then
                ...
                echo "  - ${PROJECT_ROOT}/target/wasm32v1-none/release/environmental_impact_tracking.wasm"
                echo "  - ${PROJECT_ROOT}/target/wasm32-unknown-unknown/release/environmental_impact_tracking.wasm"
                echo "  - ${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/environmental_impact_tracking.wasm"
...
print_usage() {
    echo "Usage: ./deploy_environmental_impact.zsh [network] [identity]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  ./deploy_environmental_impact.zsh testnet                  # Deploy to testnet with default identity"
    echo "  ./deploy_environmental_impact.zsh testnet alice            # Deploy to testnet with 'alice' identity"
    echo "  ./deploy_environmental_impact.zsh mainnet production       # Deploy to mainnet with production identity"
    echo ""
    echo "Prerequisites:"
    ...
    echo "  1. Build the environmental impact tracking contract"
