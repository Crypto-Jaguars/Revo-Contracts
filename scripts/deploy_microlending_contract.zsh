...
show_usage() {
    ...
    echo -e "  ${CYAN}./scripts/deploy_microlending_contract.zsh [network] [profile]${NC}"
    ...
    echo -e "  ${CYAN}./scripts/deploy_microlending_contract.zsh testnet kennyv4${NC}"
    echo -e "  ${CYAN}./scripts/deploy_microlending_contract.zsh mainnet production${NC}"
}

# Check if parameters are provided
if [ $# -ne 2 ]; then
    print_header "Microlending Contract Deployment"
...
# Microlending contract deployment
print_header "Deploying Microlending Contract"

log_step "Uploading microlending_contract to $NETWORK..."
microlending_contract_upload_output=$(stellar contract upload \
  --network $NETWORK \
  --source $PROFILE \
  --wasm target/wasm32v1-none/release/micro_lending.wasm)
...
log_info "WASM Hash: ${BOLD}$microlending_contract_upload_output${NC}"

log_step "Deploying microlending_contract..."
microlending_contract_deploy_output=$(stellar contract deploy \
  --wasm-hash $microlending_contract_upload_output \
  --source $PROFILE \
  --network $NETWORK \
  --alias MicrolendingContract)
...
log_success "Microlending Contract deployed successfully!"
log_info "Contract ID: ${BOLD}$microlending_contract_deploy_output${NC}"
log_info "Network: ${BOLD}$NETWORK${NC}"
log_info "Profile: ${BOLD}$PROFILE${NC}"
