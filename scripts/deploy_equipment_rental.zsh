...
# Equipment Rental Contract Deployment Script
# Usage: ./deploy_equipment_rental.zsh [network] [identity]
# Example: ./deploy_equipment_rental.zsh testnet default
...
CONTRACT_DIR="${PROJECT_ROOT}/ContractsRevo/equipment-rental-contract"
# Modern Stellar CLI uses workspace root target directory with wasm32v1-none
WASM_PATH="${PROJECT_ROOT}/target/wasm32v1-none/release/equipment_rental_contract.wasm"
...
setup_logging() {
    ...
    # Initialize log file
    echo "Equipment Rental Contract Deployment Log" > "$DEPLOYMENT_LOG"
...
build_contract() {
    ...
    if [[ ! -f "$WASM_PATH" ]]; then
        ...
        ALT_WASM_PATH="${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/equipment_rental_contract.wasm"
        if [[ -f "$ALT_WASM_PATH" ]]; then
            ...
            ALT_WASM_PATH2="${PROJECT_ROOT}/target/wasm32-unknown-unknown/release/equipment_rental_contract.wasm"
            if [[ -f "$ALT_WASM_PATH2" ]]; then
                ...
                echo "  - ${PROJECT_ROOT}/target/wasm32v1-none/release/equipment_rental_contract.wasm"
                echo "  - ${PROJECT_ROOT}/target/wasm32-unknown-unknown/release/equipment_rental_contract.wasm"
                echo "  - ${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/equipment_rental_contract.wasm"
...
Equipment Rental Contract Deployment Summary
===========================================
...
display_results() {
    ...
    echo "  3. Register equipment and create rental listings"
    echo "  4. Test rental and maintenance workflows"
    echo ""
...
print_usage() {
    echo "Usage: ./deploy_equipment_rental.zsh [network] [identity]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  ./deploy_equipment_rental.zsh testnet                  # Deploy to testnet with default identity"
    echo "  ./deploy_equipment_rental.zsh testnet alice            # Deploy to testnet with 'alice' identity"
    echo "  ./deploy_equipment_rental.zsh mainnet production       # Deploy to mainnet with production identity"
    echo ""
    echo "Prerequisites:"
    ...
    echo "  1. Build the equipment rental contract"
    ...
    echo "  - Register and manage equipment inventory"
    echo "  - Handle equipment rental bookings"
    echo "  - Track equipment maintenance schedules"
    echo "  - Manage pricing and availability"
}
