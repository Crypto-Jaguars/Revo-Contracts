...
# Agricultural Auction Contract Deployment Script
# Usage: ./deploy_agricultural_auction.zsh [network] [identity]
# Example: ./deploy_agricultural_auction.zsh testnet default
...
CONTRACT_DIR="${PROJECT_ROOT}/ContractsRevo/agricultural-auction-contract"
# Modern Stellar CLI uses workspace root target directory with wasm32v1-none
WASM_PATH="${PROJECT_ROOT}/target/wasm32v1-none/release/agricultural_auction_contract.wasm"
...
setup_logging() {
    ...
    # Initialize log file
    echo "Agricultural Auction Contract Deployment Log" > "$DEPLOYMENT_LOG"
...
build_contract() {
    ...
    if [[ ! -f "$WASM_PATH" ]]; then
        ...
        ALT_WASM_PATH="${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/agricultural_auction_contract.wasm"
        if [[ -f "$ALT_WASM_PATH" ]]; then
            ...
            ALT_WASM_PATH2="${PROJECT_ROOT}/target/wasm32-unknown-unknown/release/agricultural_auction_contract.wasm"
            if [[ -f "$ALT_WASM_PATH2" ]]; then
                ...
                echo "  - ${PROJECT_ROOT}/target/wasm32v1-none/release/agricultural_auction_contract.wasm"
                echo "  - ${PROJECT_ROOT}/target/wasm32-unknown-unknown/release/agricultural_auction_contract.wasm"
                echo "  - ${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/agricultural_auction_contract.wasm"
...
print_usage() {
    echo "Usage: ./deploy_agricultural_auction.zsh [network] [identity]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  ./deploy_agricultural_auction.zsh testnet                  # Deploy to testnet with default identity"
    echo "  ./deploy_agricultural_auction.zsh testnet alice            # Deploy to testnet with 'alice' identity"
    echo "  ./deploy_agricultural_auction.zsh mainnet production       # Deploy to mainnet with production identity"
    echo ""
    echo "Prerequisites:"
    ...
    echo "  1. Build the agricultural auction contract"
