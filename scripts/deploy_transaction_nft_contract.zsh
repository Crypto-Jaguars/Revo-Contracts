...
# Transaction NFT Contract Deployment Script
# Usage: ./deploy_transaction_nft_contract.zsh [network] [identity]
# Example: ./deploy_transaction_nft_contract.zsh testnet default
...
# Configuration
CONTRACT_NAME="transaction-nft-contract"
SCRIPT_DIR="${0:A:h}"
PROJECT_ROOT="${SCRIPT_DIR:h}"
CONTRACT_DIR="${PROJECT_ROOT}/ContractsRevo/transaction-nft-contract"
# Modern Stellar CLI uses workspace root target directory with wasm32v1-none
WASM_PATH="${PROJECT_ROOT}/target/wasm32v1-none/release/transaction_nft_contract.wasm"
...
setup_logging() {
    ...
    # Initialize log file
    echo "Transaction NFT Contract Deployment Log" > "$DEPLOYMENT_LOG"
...
build_contract() {
    ...
    if [[ ! -f "$WASM_PATH" ]]; then
        ...
        ALT_WASM_PATH="${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/transaction_nft_contract.wasm"
        if [[ -f "$ALT_WASM_PATH" ]]; then
            ...
            echo "  - ${PROJECT_ROOT}/target/wasm32v1-none/release/transaction_nft_contract.wasm"
            echo "  - ${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/transaction_nft_contract.wasm"
...
upload_contract() {
    ...
    # The upload command outputs the hash (may be first or second line depending on if already uploaded)
...
deploy_contract() {
    ...
    # The deploy command outputs the contract ID (usually last line matching the pattern)
...
Transaction NFT Contract Deployment Summary
=================================================
...
display_results() {
    ...
    echo "  2. Test contract functionality (mint_nft, get_nft_metadata)"
    echo "  3. Save contract ID for client integration"
    echo ""
    echo -e "${CYAN}Contract Functions:${NC}"
    echo "  - mint_nft(buyer, seller, amount, product)"
    echo "  - get_nft_metadata(tx_id)"
    echo ""
...
print_usage() {
    echo "Usage: ./deploy_transaction_nft_contract.zsh [network] [identity]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  ./deploy_transaction_nft_contract.zsh testnet                    # Deploy to testnet with default identity"
    echo "  ./deploy_transaction_nft_contract.zsh testnet alice              # Deploy to testnet with 'alice' identity"
    echo "  ./deploy_transaction_nft_contract.zsh mainnet production         # Deploy to mainnet with production identity"
    echo ""
    echo "Prerequisites:"
    ...
    echo "  - Funded account for transaction fees"
    echo ""
    echo "Setup Identity:"
    ...
    echo "  1. Build the transaction NFT contract"
    ...
    echo "  - mint_nft(buyer, seller, amount, product) - Mint transaction NFT"
    echo "  - get_nft_metadata(tx_id) - Retrieve NFT metadata"
}
