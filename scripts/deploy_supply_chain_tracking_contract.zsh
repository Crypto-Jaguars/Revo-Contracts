...
show_usage() {
    ...
    echo -e "  ${CYAN}./scripts/deploy_supply_chain_tracking_contract.zsh [network] [profile]${NC}"
    ...
    echo -e "  ${CYAN}./scripts/deploy_supply_chain_tracking_contract.zsh testnet kennyv4${NC}"
    echo -e "  ${CYAN}./scripts/deploy_supply_chain_tracking_contract.zsh mainnet production${NC}"
}

# Check if parameters are provided
if [ $# -ne 2 ]; then
    print_header "Supply Chain Tracking Contract Deployment"
...
# Supply chain tracking contract deployment
print_header "Deploying Supply Chain Tracking Contract"

log_step "Uploading supply_chain_tracking_contract to $NETWORK..."
supply_chain_tracking_contract_upload_output=$(stellar contract upload \
  --network $NETWORK \
  --source $PROFILE \
  --wasm target/wasm32v1-none/release/supply_chain_tracking_contract.wasm)
...
log_info "WASM Hash: ${BOLD}$supply_chain_tracking_contract_upload_output${NC}"

log_step "Deploying supply_chain_tracking_contract..."
supply_chain_tracking_contract_deploy_output=$(stellar contract deploy \
  --wasm-hash $supply_chain_tracking_contract_upload_output \
  --source $PROFILE \
  --network $NETWORK \
  --alias SupplyChainTrackingContract)
...
log_success "Supply Chain Tracking Contract deployed successfully!"
log_info "Contract ID: ${BOLD}$supply_chain_tracking_contract_deploy_output${NC}"
log_info "Network: ${BOLD}$NETWORK${NC}"
log_info "Profile: ${BOLD}$PROFILE${NC}"
