...
# Cooperative Management Contract Deployment Script
# Usage: ./deploy_cooperative_management.zsh [network] [identity]
# Example: ./deploy_cooperative_management.zsh testnet default
...
CONTRACT_DIR="${PROJECT_ROOT}/ContractsRevo/cooperative-management-contract"
# Stellar CLI uses workspace root target directory with wasm32v1-none
WASM_PATH="${PROJECT_ROOT}/target/wasm32v1-none/release/cooperative_management_contract.wasm"
...
setup_logging() {
    ...
    # Initialize log file
    echo "Cooperative Management Contract Deployment Log" > "$DEPLOYMENT_LOG"
...
build_contract() {
    ...
    if [[ ! -f "$WASM_PATH" ]]; then
        ...
        ALT_WASM_PATH="${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/cooperative_management_contract.wasm"
        if [[ -f "$ALT_WASM_PATH" ]]; then
            ...
            ALT_WASM_PATH2="${PROJECT_ROOT}/target/wasm32-unknown-unknown/release/cooperative_management_contract.wasm"
            if [[ -f "$ALT_WASM_PATH2" ]]; then
                ...
                echo "  - ${PROJECT_ROOT}/target/wasm32v1-none/release/cooperative_management_contract.wasm"
                echo "  - ${PROJECT_ROOT}/target/wasm32-unknown-unknown/release/cooperative_management_contract.wasm"
                echo "  - ${CONTRACT_DIR}/target/wasm32-unknown-unknown/release/cooperative_management_contract.wasm"
...
print_usage() {
    echo "Usage: ./deploy_cooperative_management.zsh [network] [identity]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  ./deploy_cooperative_management.zsh testnet                  # Deploy to testnet with default identity"
    echo "  ./deploy_cooperative_management.zsh testnet alice            # Deploy to testnet with 'alice' identity"
    echo "  ./deploy_cooperative_management.zsh mainnet production       # Deploy to mainnet with production identity"
    echo ""
    echo "Prerequisites:"
    ...
    echo "  1. Build the cooperative management contract"
