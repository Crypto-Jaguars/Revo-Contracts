...
show_usage() {
    ...
    echo -e "  ${CYAN}./scripts/deploy_land_leasing_contract.zsh [network] [profile]${NC}"
    ...
    echo -e "  ${CYAN}./scripts/deploy_land_leasing_contract.zsh testnet kennyv4${NC}"
    echo -e "  ${CYAN}./scripts/deploy_land_leasing_contract.zsh mainnet production${NC}"
}

# Check if parameters are provided
if [ $# -ne 2 ]; then
    print_header "Land Leasing Contract Deployment"
...
# Land Leasing Contract deployment
print_header "Deploying Land Leasing Contract"

log_step "Uploading land_leasing_contract to $NETWORK..."
land_leasing_contract_upload_output=$(stellar contract upload \
  --network $NETWORK \
  --source $PROFILE \
  --wasm target/wasm32v1-none/release/land_leasing_contract.wasm)
...
log_info "WASM Hash: ${BOLD}$land_leasing_contract_upload_output${NC}"

log_step "Deploying land_leasing_contract..."
land_leasing_contract_deploy_output=$(stellar contract deploy \
  --wasm-hash $land_leasing_contract_upload_output \
  --source $PROFILE \
  --network $NETWORK \
  --alias LandLeasingContract)
...
log_success "Land Leasing Contract deployed successfully!"
log_info "Contract ID: ${BOLD}$land_leasing_contract_deploy_output${NC}"
log_info "Network: ${BOLD}$NETWORK${NC}"
log_info "Profile: ${BOLD}$PROFILE${NC}"
