...
show_usage() {
    ...
    echo -e "  ${CYAN}./scripts/deploy_farmer_token_contract.zsh [network] [profile]${NC}"
    ...
    echo -e "  ${CYAN}./scripts/deploy_farmer_token_contract.zsh testnet kennyv4${NC}"
    echo -e "  ${CYAN}./scripts/deploy_farmer_token_contract.zsh mainnet production${NC}"
}

# Check if parameters are provided
if [ $# -ne 2 ]; then
    print_header "Farmer Token Contract Deployment"
...
# Farmer Token Contract deployment
print_header "Deploying Farmer Token Contract"

log_step "Uploading farmer_token_contract to $NETWORK..."
farmer_token_contract_upload_output=$(stellar contract upload \
  --network $NETWORK \
  --source $PROFILE \
  --wasm target/wasm32v1-none/release/farmer_token_contract.wasm)
...
log_info "WASM Hash: ${BOLD}$farmer_token_contract_upload_output${NC}"

log_step "Deploying farmer_token_contract..."
farmer_token_contract_deploy_output=$(stellar contract deploy \
  --wasm-hash $farmer_token_contract_upload_output \
  --source $PROFILE \
  --network $NETWORK \
  --alias FarmerTokenContract)
...
log_success "Farmer Token Contract deployed successfully!"
log_info "Contract ID: ${BOLD}$farmer_token_contract_deploy_output${NC}"
log_info "Network: ${BOLD}$NETWORK${NC}"
log_info "Profile: ${BOLD}$PROFILE${NC}"
